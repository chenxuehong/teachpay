...
public class CalendarDialogFragment extends AppCompatDialogFragment {
    ...
    public void onViewClicked(View view) {
        if (view.getId() == R.id.dialogfragment_calendar_iv_lastMonth) {
            ...
            if (checkedDayList != null && checkedDayList.size() > 0) {
                Collections.sort(checkedDayList);
            }
            EventBusUtils.sendEvent(new Event(EventAction.CALENDAR_DAY_LIST, checkedDayList));
            dismiss();
        }
    }
    ...
    private int getLayoutId() {
        return R.layout.dialogfragment_calendar;
    }
    ...
    private void initView() {
        ...
        String curDateStr = DateUtils.getCurDateStr(DateUtils.YMFormatStr3);
        String curDateStr2 = DateUtils.getCurDateStr(DateUtils.YMDFormatStr4);
        String curDateStr3 = DateUtils.getCurDateStr(DateUtils.DFormatStr2);
        int curYear = DateUtils.getCurYear();
        int startYear = curYear - 3;
        int endYear = curYear + 1;
        String startYearStr = String.valueOf(startYear).concat(".1");
        String endYearStr = String.valueOf(endYear).concat(".12");
        ArrayList<String> strings = new ArrayList<String>();
        checkedDayList.add(curDateStr3);
        strings.add(curDateStr2);
        //日历init，年月日之间用点号隔开
        calendarView
                .setStartEndDate(startYearStr, endYearStr)
                .setInitDate(curDateStr)
                .setMultiDate(strings)
                .init();
        tvTitle.setText(curDateStr);
        //月份切换回调
        ...
        calendarView.setOnMultiChooseListener(new OnMultiChooseListener() {
            ...
            public void onMultiChoose(View view, DateBean date, boolean flag) {
                ...
                int day = solar[solar.length - 1];
                if (flag) {
                    if (!checkedDayList.contains(String.valueOf(day))) {
                        checkedDayList.add(String.valueOf(day));
                    }
                } else {
                    checkedDayList.remove(String.valueOf(day));
                }

                if (checkedDayList != null) {
                    StringBuilder stringBuilder = new StringBuilder();
                    for (int i = 0; i < checkedDayList.size(); i++) {
                        stringBuilder.append(checkedDayList.get(i)).append(" ");
                    }
