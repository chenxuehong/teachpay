...
public class ScheduleMechanismWaitingClassRvAdapter extends PinnedHeaderAdapter<MechanismOfflineScheduleEntity> {

    private static final int VIEW_TYPE_ITEM_TIME = 0;
    private static final int VIEW_TYPE_ITEM_CONTENT = 1;

    public ScheduleMechanismWaitingClassRvAdapter(int layoutId, Context context) {
    ...
    public void hideView(View pinnedHeaderView) {
        pinnedHeaderView.findViewById(R.id.item_mechanism_course_waiting_timeline_ll).setVisibility(View.GONE);
    }
...
//        end_time = DateUtils.translateZoneTimeStr(end_time, mechanismOfflineScheduleEntity.getOffset(), DateUtils.getCurTimeZoneOffset(), DateUtils.YMDHMSFormatStr);
    ...
    protected void convert(ViewHolder viewHolder, MechanismOfflineScheduleEntity mechanismOfflineScheduleEntity, int position) {
        if (getItemViewType(position) == VIEW_TYPE_ITEM_CONTENT) {
            viewHolder.setVisible(R.id.item_mechanism_course_waiting_timeline_ll_timetitle, false);
        } else {
            viewHolder.setVisible(R.id.item_mechanism_course_waiting_timeline_ll_timetitle, true);
        }
        ...
        if (map != null) {
            ...
            if (masterInfo != null) {
                // 上课老师信息
                CircleImageView ivHead = viewHolder.getView(R.id.item_mechanism_course_waiting_iv_head);
                GlideTools.loadImage(context, masterInfo.getAvatar(), ivHead);
                viewHolder.setText(R.id.item_mechanism_course_waiting_tv_nickName, masterInfo.getNick_name());
            }
            MasterSetPriceEntity masterSetPriceEntity = map.getMasterSetPriceEntity();
            if (masterSetPriceEntity != null) {
                viewHolder.setText(R.id.item_mechanism_course_waiting_tv_title, masterSetPriceEntity.getTitle());
            }
        ...
        viewHolder.setVisible(R.id.item_mechanism_course_waiting_tv_isToday, isToday);

        // 课程信息
        String title = mechanismOfflineScheduleEntity.getTitle();
        String classroom = mechanismOfflineScheduleEntity.getClassroom();
        viewHolder.setText(R.id.item_mechanism_course_waiting_tv_childTitle, title);
        viewHolder.setText(R.id.item_mechanism_course_waiting_tv_classroom, classroom);
        String end_time = mechanismOfflineScheduleEntity.getEnd_time();
        start_time = DateUtils.translateZoneTimeStr(start_time, mechanismOfflineScheduleEntity.getOffset(), DateUtils.getCurTimeZoneOffset(), DateUtils.YMDHMSFormatStr);
        end_time = DateUtils.translateZoneTimeStr(end_time, mechanismOfflineScheduleEntity.getOffset(), DateUtils.getCurTimeZoneOffset(), DateUtils.YMDHMSFormatStr);
        ...
        viewHolder.setText(R.id.item_mechanism_course_waiting_timeline_tv_timetitle, week);
        viewHolder.setText(R.id.item_mechanism_course_waiting_timeline_tv_month, ymd);
        viewHolder.setText(R.id.item_mechanism_course_waiting_timeline_tv_duration, startHM.concat("\n-\n").concat(end_HM));
        viewHolder.setText(R.id.item_mechanism_course_waiting_tv_summary, "总结");
        viewHolder.setOnClickListener(R.id.item_mechanism_course_waiting_tv_summary,
                new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        ...
                        ActivityToActivity.toActivity(ARouterConfig.SCHEDULE_SUMMARYACTIVITY, params);
                    }
                });
    }
}
