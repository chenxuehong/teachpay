...
import com.huihe.base_lib.model.base.Template;
...
public class ActivityToActivity {
    ...
    public static void toWebView(Context activity, String url) {
        H5TitleActivity.actionStart(activity, url);
    }
        ...
        if (params != null) {
            for (Map.Entry<String, ?> entry : params.entrySet()) {
                ...
                if (value instanceof String) {
                    ...
                    BaseBean value1 = (BaseBean) value;
                    postcard.withSerializable(key,value1);
                } else if (value instanceof ArrayList) {
