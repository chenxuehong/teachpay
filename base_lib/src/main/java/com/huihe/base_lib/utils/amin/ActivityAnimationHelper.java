...
public class ActivityAnimationHelper {
    ...
    private static void calculatePivotXY(View view, int[] location, Context context) {
        int screenCenterX = DensityUtils.getScreenWidth(context) / 2;
        int screenCenterY = DensityUtils.getScreenHeight(context) / 2;
        if (view != null) {
    ...
    public static void animScaleUp(BaseAnimActivity activity, Intent intent) {
     ...
     */
    public static void animScaleUp(final BaseAnimActivity activity, Intent intent,
        ...
        int screenCenterX = DensityUtils.getScreenWidth(activity) / 2;
        int screenCenterY = DensityUtils.getScreenHeight(activity) / 2;
    ...
    public static void animScaleDown(final BaseAnimActivity activity, final OnAnimationListener listener) {
        if (!canAnimation(activity)) {
            notifyAnimState(listener, ANIM_STATE_END);
            return;
        }
