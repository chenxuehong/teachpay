...
/**
 * @author Wlq
 * @description 字符串过滤emoji表情
 * @date 2017/12/14 上午9:46
 */
    ...
    private static boolean isEmojiCharacter(int first) {
       ...
       /* 1F30D - 1F567
        ...
        return !((first == 0x0) ||
                ...
                ((first >= 0x10000))) ||
                (first == 0xa9 || first == 0xae || first == 0x2122 || first == 0x3030 ||
                        (first >= 0x25b6 && first <= 0x27bf) || first == 0x2328 ||
                        (first >= 0x23e9 && first <= 0x23fa)) ||
