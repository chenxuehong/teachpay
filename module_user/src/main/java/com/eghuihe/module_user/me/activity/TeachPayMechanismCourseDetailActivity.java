 ...
 * @desc 教付保 机构课程详情
 */
@Route(path = ARouterConfig.ME_MECHANISM_COURSE_DETAIL)
public class TeachPayMechanismCourseDetailActivity extends BaseMvpActivity<TeachPayMechanismCourseDetailPresenter>
        implements TeachPayMechanismCourseDetailContract.View {

    public static final String KEY_DATA = ArgumentsConfig.MECHANISM_COURSE_DETAIL;
    @BindView(R2.id.activity_course_detail_onsale_SmartRefreshLayout)
    SmartRefreshLayout smartRefreshLayout;
    @BindView(R2.id.activity_course_detail_onsale_rv)
    RecyclerViewFixed recyclerViewFixed;
    @BindView(R2.id.activity_live_course_fl_status)
    FrameLayout flStatus;
    @BindView(R2.id.activity_course_detail_onsale_ll_pay)
    LinearLayout llPay;
    @BindView(R2.id.activity_course_detail_onsale_iv_pay)
    TextView tvSignUp;
    @BindView(R2.id.layout_teach_pay_course_onsale_head_iv_living)
    ImageView ivLiviing;
    ...
    private BaseDialog mechanismCourseBuySuccDialog;
    ...
    @OnClick({
            ...
            R2.id.activity_course_detail_onsale_iv_pay,
            R2.id.activity_course_detail_onsale_tv_consult,
            R2.id.activity_live_course_fl_share,
            R2.id.activity_course_detail_onsale_iv_discount_pay
    })
    public void onViewClicked(View view) {
        if (!EventUtils.isFastDoubleClick(view.getId())) {
            if (view.getId() == R.id.activity_live_course_fl_titleBack) {
                finish();
            } else if (view.getId() == R.id.activity_course_detail_onsale_iv_pay) {
                requestBuy();
            } else if (view.getId() == R.id.activity_course_detail_onsale_tv_consult) {
                consult();
            } else if (view.getId() == R.id.activity_course_detail_onsale_iv_discount_pay) {
                questDiscountBuy();
            } else if (view.getId() == R.id.activity_live_course_fl_share) {
                // 分享 课程详情
                share();
            }
        }
    ...
    private void requestBuy() {
        ...
        exclusiveCoursesEntity.setIs_one_time_payment(true);
        params.put(ArgumentsConfig.KEY_MASTERSETPRICEENTITY, JsonUtil.toJson(exclusiveCoursesEntity));
        ActivityToActivity.toActivity(ARouterConfig.SCHEDULE_LIVECOURSEPAYACTIVITY, params);
    }

    /**
     * @desc 全额支付
     */
    private void questDiscountBuy() {
        Map<String, String> params = new HashMap<>();
        exclusiveCoursesEntity.setIs_one_time_payment(false);
        params.put(ArgumentsConfig.KEY_MASTERSETPRICEENTITY, JsonUtil.toJson(exclusiveCoursesEntity));
        ActivityToActivity.toActivity(ARouterConfig.SCHEDULE_LIVECOURSEPAYACTIVITY, params);
    }
    ...
    private void share() {
        ...
        map.put(ArgumentsConfig.KEY_WXPATH,
                AppConfigs.Cooperation.ZH.COURSE_DETAIL_WX_PATH
                        .concat(exclusiveCoursesEntity.getId()));
        EventBusUtils.sendEvent(new Event(EventAction.COURSE_DETAIL_SHARE, map));
    }
    ...
    protected void initData() {
        ...
        String live_streaming_id = exclusiveCoursesEntity.getLive_streaming_id();
        boolean isLiving = !TextUtils.isEmpty(live_streaming_id) && !"0".equals(live_streaming_id);
        if (!isLiving) {
            getPresenter().queryMechanismCourseListById(exclusiveCoursesEntity.getId());
        } else {
            getPresenter().queryMechanismMasterSetPriceDisplay(exclusiveCoursesEntity.getId(), live_streaming_id);
        }
...
//                ActivityToActivity.toActivity(ARouterConfig.HOME_NEWACTIVITYDETAILACTIVITY, params);
                // 购买抵扣劵
                Map<String, String> params = new HashMap<>();
                params.put(ArgumentsConfig.KEY_TYPE, "deductions_coupon");
    ...
    private void initHeadView(View headView) {
        ...
        ivLiviing.setVisibility(isLiving ? View.VISIBLE : View.GONE);
        if (isLiving) {
            GlideTools.loadGifImage(this, R.mipmap.living_float, ivLiviing);
            getPresenter().queryMasterSetPriceStreamDetail(live_streaming_id);
        }
        ...
        ivCourseCover.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (!TextUtils.isEmpty(cover)) {
                    Intent intent = new Intent(TeachPayMechanismCourseDetailActivity.this, PhotoViewActivity.class);
        ...
        // 直播总价
        Double live_stream_price = exclusiveCoursesEntity.getLive_stream_price();
        live_stream_price = live_stream_price == null ? 0.0 : live_stream_price;
        // 直播单价
        Double living_single_session_price = exclusiveCoursesEntity.getLiving_single_session_price();
        living_single_session_price = living_single_session_price == null ? 0.0 : living_single_session_price;
        if (isLiving) {
            ...
            tvDiscoverAmount.setText(NumberUtils.transMoney(String.valueOf(live_stream_price)));
            if (living_single_session_price > 0) {
                tvSinglePaymentAmount.setText(NumberUtils.transMoney(String.valueOf(living_single_session_price)).concat("/节"));
                tvSinglePaymentAmount.setVisibility(View.VISIBLE);
                tvSignUp.setVisibility(View.VISIBLE);
                tvSignUp.setText("单节付报名");
            } else {
    ...
    private void enterLiveRoom(LiveDetailEntity liveDetailEntity) {
        MasterSetPriceEntity.Map map = exclusiveCoursesEntity.getMap();
        if (map != null) {
            MasterMechanismModel.MasterMechanismEntity masterMechanismEntity = map.getMasterMechanismEntity();
            if (masterMechanismEntity != null) {
                String id = masterMechanismEntity.getId();
                liveDetailEntity.setRoom_id(id);
                TUIKit.enterLive(this, JsonUtil.toJson(liveDetailEntity));
            }
        }
    }
    ...
    public void onGetAliOrderInfo(String orderInfo) {
        AlipayUtils.getInstance(TeachPayMechanismCourseDetailActivity.this).pay(orderInfo, null);
    }
...
//        WeiXinUtils.startWechatPay(wxPayEntity);
    ...
    @Override
    public void onMasterSetPriceStreamDetail(LiveDetailEntity liveDetailEntity) {
        if (ivLiviing != null)
            ivLiviing.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    enterLiveRoom(liveDetailEntity);
                }
            });
    }
    ...
    @Override
    public void onPayOneCourseOrder(StudentCoursePackageEntity studentCoursePackageEntity) {

        mechanismCourseBuySuccDialog = MyDialogUtils.showMechanismCourseBuySuccessfulDialog(
                this, exclusiveCoursesEntity,
                new MyDialogUtils.OnCourseBuySuccListener() {

                    @Override
                    public void onGoAppointment() {
                        String title = exclusiveCoursesEntity.getTitle();
                        String titile_url = exclusiveCoursesEntity.getTitile_url();
                        Map<String, String> params = new HashMap<>();
                        params.put(ArgumentsConfig.KEY_STUDENTCOURSEPACKAGEENTITY,
                                JsonUtil.toJson(studentCoursePackageEntity));
                        params.put(ArgumentsConfig.KEY_ID, studentCoursePackageEntity.getId());
                        params.put(ArgumentsConfig.KEY_TITLE, title);
                        params.put(ArgumentsConfig.KEY_TITLE_URL, titile_url);
                        ActivityToActivity.toActivity(
                                ARouterConfig.SCHEDULE_TEACHPAYSTUDENTSCHEDULEPACKAGEDETAILLISTACTIVITY,
                                params);
                    }

                    @Override
                    public void onGoPinGroup() {
    ...
    @Override
    public void onPingGroupSuccess(String content) {

        Intent intent3 = new Intent(this, H5TitleActivity.class);
        intent3.putExtra(H5TitleActivity.KET_URL,
                AppConfigs.Cooperation.ZH.PING_TUAN.concat(content));
    ...
    @Override
    protected TeachPayMechanismCourseDetailPresenter createPresenter() {
        return new TeachPayMechanismCourseDetailPresenter();
    }

    @Override
    protected void onDestroy() {
        try {
            if (mechanismCourseBuySuccDialog != null) {
                mechanismCourseBuySuccDialog.dismiss();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
