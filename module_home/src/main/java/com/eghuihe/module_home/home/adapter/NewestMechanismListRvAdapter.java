...
public class NewestMechanismListRvAdapter extends EmptyRVAdapter<MasterMechanismModel.MasterMechanismEntity> {
    public NewestMechanismListRvAdapter(int layoutId, Context context) {
    ...
    protected void convert(ViewHolder viewHolder, MasterMechanismModel.MasterMechanismEntity masterMechanismEntity, int position) {
        RoundedImageView ivMechanismLogo = viewHolder.getView(R.id.item_newest_mechanism_course_iv_cover);
        AppCompatRatingBar appCompatRatingBar = viewHolder.getView(R.id.item_newest_mechanism_course_ratingBar);
        ...
        viewHolder.setText(R.id.item_newest_mechanism_course_tv_mechanism_name,
                mechanism_name);
        ...
        viewHolder.setText(R.id.item_newest_mechanism_course_tv_address,
                mechanism_addr);
        viewHolder.setText(R.id.item_newest_mechanism_course_tv_distance,
                distanceStr);
        viewHolder.setVisible(R.id.item_newest_mechanism_course_rv_Recommend, false);
        viewHolder.setVisible(R.id.item_newest_mechanism_course_tv_See_more_courses, false);
        ...
        if (map != null) {
            ...
            if (masterSetPriceEntities != null && masterSetPriceEntities.size() > 2) {
                viewHolder.setVisible(R.id.item_newest_mechanism_course_tv_See_more_courses, true);
            }
            if (masterSetPriceEntities != null && masterSetPriceEntities.size() > 0) {
                viewHolder.setVisible(R.id.item_newest_mechanism_course_rv_Recommend, true);
                initRecommendCourseRvAdapter(viewHolder, masterSetPriceEntities);
            }
        }
        viewHolder.setText(R.id.item_newest_mechanism_course_tv_categories, categoriesChild);
        ...
        viewHolder.setOnClickListener(R.id.item_newest_mechanism_course_tv_See_more_courses, new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                // 更多机构课程
    ...
    private void initRecommendCourseRvAdapter(ViewHolder viewHolder, List<MasterSetPriceEntity> masterSetPriceEntities) {
        ...
        RecyclerViewFixed rvRecommendCourses = viewHolder.getView(R.id.item_newest_mechanism_course_rv_Recommend);
