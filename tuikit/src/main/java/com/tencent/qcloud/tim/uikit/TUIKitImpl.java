...
public class TUIKitImpl {
    ...
    public static void login(final String userid, final String usersig, final IUIKitCallBack callback) {
        ...
        V2TIMManager.getInstance().login(userid, usersig, new V2TIMCallback() {
            ...
            public void onError(int code, String desc) {
                LogUtils.i(TAG, "login error===>code = "
    ...
    public static void startLive(Context context, String data) {
        try {
            Class<?> classz = Class.forName("com.tencent.qcloud.tim.tuikit.live.modules.liveroom.ui.DefaultGroupLiveAnchorActivity");
            Method method = classz.getMethod("start", Context.class, String.class);
            method.invoke(null, context,data);
        } catch (ClassNotFoundException | NoSuchMethodException | IllegalAccessException | InvocationTargetException e) {
            TUIKitLog.e(TAG, "startLive error: " + e.getMessage());
        }
    }

    public static void enterLive(Context context, String data) {
        try {
            Class<?> classz = Class.forName("com.tencent.qcloud.tim.tuikit.live.modules.liveroom.ui.DefaultGroupLiveAudienceActivity");
            Method method = classz.getMethod("start", Context.class, String.class);
            method.invoke(null, context,  data);
        } catch (ClassNotFoundException | NoSuchMethodException | IllegalAccessException | InvocationTargetException e) {
            TUIKitLog.e(TAG, "enterLive error: " + e.getMessage());
        }
    }
